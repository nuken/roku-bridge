"""
A specific plugin implementation for the FOX One app (Roku App ID: 808732).
This app often requires navigating to a specific live event from a list.
"""
from .base_plugin import BaseAppPlugin
import logging

class FoxOnePlugin(BaseAppPlugin):
    """
    App-specific plugin for the FOX One app.
    """

    def __init__(self):
        # The official Roku App ID for FOX One is "808732"
        super().__init__(app_id="808732", app_name="FOX One")

    def tune_channel(self, roku_ip, channel_data):
        """
        Implements the tuning logic for the FOX One app.
        ...
        sequence = [
            ...
            {"wait": 3}, # Wait for the app to load
        ...
        if list_position > 1:
            ...
            for _ in range(list_position - 1):
                ...
                sequence.append({"wait": 0.5}) # Small delay between presses

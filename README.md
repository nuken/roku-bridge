...
The page will confirm the upload was successful and the server will reload automatically, showing the status of your newly configured tuners. Your setup is now complete.
...
## **Configuration (roku_channels.json)**
...
* **`encoding_mode`**: **(Optional)** Sets the stream handling mode for this specific tuner. Can be `proxy`, `remux`, or `reencode`. If omitted, it will use the global `ENCODING_MODE` environment variable.
...
"tuners": [
  ...
  {
    "name": "Roku 2 (Re-encode)",
    "roku_ip": "192.168.1.11",
    "encoder_url": "http://192.168.1.21/ts/1_0",
    "priority": 2,
    "encoding_mode": "reencode"
  }
]
...
You can set a global, default stream handling mode by adding the `-e ENCODING_MODE=<mode>` flag to your `docker run` command. This setting will be used for any tuner that does **not** have a specific `encoding_mode` set in the `roku_channels.json` file.
